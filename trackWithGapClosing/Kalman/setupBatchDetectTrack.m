...
for iJob = 1 : numJobs
    ...
    baseDirectory = input('Movie directory name (END WITH PROPER SLASH): ');
    experimentName = input('File name base: ');
    firstImage = input('First image to analyze: ');
    lastImage = input('Last image to analyse: ');
    
    %movie information
    movieParam.imageDir = baseDirectory; %directory where images are
    movieParam.filenameBase = experimentName; %image file name base
    movieParam.firstImageNum = firstImage; %number of first image in movie % 1- 397 & 398-794
    movieParam.lastImageNum = lastImage; %number of last image in movie
    movieParam.digits4Enum = 4; %number of digits used for frame enumeration (1-4).
    
    %save detection results
    saveResultsDetect.dir = baseDirectory; %directory where to save input and output
    saveResultsDetect.filename = [experimentName '_detection.mat']; %name of file where input and output are saved
    
    %save tracking results
    saveResultsTrack.dir = baseDirectory; %directory where to save input and output
    saveResultsTrack.filename = [experimentName '_tracks.mat']; %name of file where input and output are saved

    %detection parameters
    detectionParam.psfSigma = 1.0; %point spread function sigma (in pixels)
    ...
    parameters.resLimit = 4.2; %resolution limit, which is generally equal to 3 * point spread function sigma.
