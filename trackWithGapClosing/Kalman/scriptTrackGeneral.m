...
parameters.minSearchRadius = 30; %minimum allowed search radius. The search radius is calculated on the spot in the code given a feature's motion parameters. If it happens to be smaller than this minimum, it will be increased to the minimum.
parameters.maxSearchRadius = 30; %maximum allowed search radius. Again, if a feature's calculated search radius is larger than this maximum, it will be reduced to this maximum.
...
%optional input
parameters.diagnostics = [2 49]; %if you want to plot the histogram of linking distances up to certain frames, indicate their numbers; 0 or empty otherwise. Does not work for the first or last frame of a movie.
...
parameters.minSearchRadius = 30; %minimum allowed search radius.
parameters.maxSearchRadius = 30; %maximum allowed search radius.
...
parameters.resLimit = 4.5; %resolution limit, which is generally equal to 3 * point spread function sigma.
...
%saveResults
saveResults.dir = '/home/kj35/1002ForNanoCourse/practicalExercises/exercise4_trackingLinearMotion/example3_highSNR_twoDirections/'; %directory where to save input and output
saveResults.filename = 'tracksNoLinDetection1_minR30_maxR30.mat'; %name of file where input and output are saved
