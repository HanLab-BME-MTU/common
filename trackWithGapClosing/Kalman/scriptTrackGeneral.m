...
%optional input
parameters.diagnostics = 18; %if you want to plot the histogram of linking distances up to certain frames, indicate their numbers; 0 or empty otherwise. Does not work for the first or last frame of a movie.
...
%saveResults
% saveResults.dir = '/home/kj35/files/LCCB/receptors/trialStuff/Bree/CrowdSourcingProblem/forKhuloud/'; %directory where to save input and output
% saveResults.filename = 'movie38TracksTest8.mat'; %name of file where input and output are saved
...
%     saveResults.filename = ['tracks2All_' sprintf('%02i',i) '.mat'];
%     [tracksFinal,kalmanInfoLink,errFlag] = trackCloseGapsKalmanSparse(movieInfoTmp,...
%         costMatrices,gapCloseParam,kalmanFunctions,probDim,saveResults,verbose);
%     clear movieInfoTmp
% end
% i=12;
% movieInfoTmp((i-1)*1200+1:14800) = movieInfo((i-1)*1200+1:14800);
% saveResults.filename = ['tracks2All_' sprintf('%02i',i) '.mat'];
% [tracksFinal,kalmanInfoLink,errFlag] = trackCloseGapsKalmanSparse(movieInfoTmp,...
%     costMatrices,gapCloseParam,kalmanFunctions,probDim,saveResults,verbose);
% clear movieInfoTmp
