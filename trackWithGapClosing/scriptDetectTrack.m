...
gapCloseParam.timeWindow = 5; %maximum allowed time gap (in frames) between a track end and a track start that allows linking them.
...
costMatParam.minSearchRadiusL = 0.2; %minimum allowed search radius (in pixels). The search radius is calculated on the spot in the code given a feature's motion parameters. If it happens to be smaller than this minimum, it will be increased to the minimum.
costMatParam.maxSearchRadiusL = 1; %maximum allowed search radius (in pixels). Again, if a feature's calculated search radius is larger than this maximum, it will be reduced to this maximum.
...
%previous step
costMatParam.minSearchRadiusCG = 0.2; %minimum allowed search radius (in pixels).
costMatParam.maxSearchRadiusCG = 1; %maximum allowed search radius (in pixels).
...
useLocalDensity.nnWindowL = 5; %number of frames before the current one where you want to look to see a feature's nearest neighbor in order to decide how isolated it is (in the initial linking step).
useLocalDensity.nnWindowCG = 5; %number of frames before/after the current one where you want to look to see a track's nearest neighbor at its end/start (in the gap closing step).

%saveResults
saveResults.dir = 'O:\08072007_NonFACS_STEALTHctr_20sec\CTR_20sec_01_1_crop\'; %directory where to save input and output
saveResults.filename = 'tracks_CTR_20sec_01_1_crop'; %name of file where input and output are saved
