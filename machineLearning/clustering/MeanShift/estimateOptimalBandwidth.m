function [H,He] = estimateOptimalBandwidth(X,HRange,varargin)
...
for ih = 1:nH
    ...
    [clInf,ptInd,ptTraj] = MeanShiftClustering(X,Htry(ih),'method','standard','minClusterDistance',Htry(ih));
    ...
    for u = 1:nC(ih)%for each cluster
        ...
        mu = clInf(u).ptClusterCenter;%Center of this cluster is assumed to be mean
        Yi = cellfun(@(x)(x(1:end-1,:)),ptTraj(ptInd == u),'Unif',0);%All traj points for this cluster, exlcluding first which is the point itself
        Yi = vertcat(Yi{:});
        Mi = cellfun(@(x)(diff(x,1,1)),ptTraj(ptInd == u),'Unif',0);
        Mi = vertcat(Mi{:});
        %tu = size(Yi,1);%Number of traj points this cluster            
        
        sig2v = nan(1,d);
        for v = 1:d
            num = sum(Mi(:,v) .* (mu(v)-Yi(:,v)));
            den = sum(Mi(:,v) .^2);
        ...
        %Assign the sigmas to each point in the cluster for per-point
        %sigmas
        %NOTE: We should actually not estimate if #pts too low...
        ppSig(ptInd == u,ih,:) = repmat(sigEst{ih}(u,:),[nnz(ptInd==u) 1]);
        ppMu(ptInd == u,ih,:) = repmat(mu,[nnz(ptInd==u) 1]);        
