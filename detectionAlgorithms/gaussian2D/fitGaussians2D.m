...
% for background estimation - always define annularMask so parfor can broadcast it
...
% Pre-allocate plain arrays so parfor can slice them
x_out      = NaN(1,np);
y_out      = NaN(1,np);
A_out      = NaN(1,np);
s_out      = NaN(1,np);
    ...
    if (xi(p)>w4 && xi(p)<=nx-w4 && yi(p)>w4 && yi(p)<=ny-w4)
        ...
        if npx >= 10 % only perform fit if window contains sufficient data points
            ...
            if (dx > -w2 && dx < w2 && dy > -w2 && dy < w2 && prm(3)<2*diff(iRange))
                ...
                sigma_r(p)    = res.std;
                RSS(p)        = res.RSS;
                SE_sigma_r(p) = res.std/sqrt(2*(npx-1));
                SE_sigma_r_kLevel = SE_sigma_r(p) * kLevel;
                ...
                mask_Ar(p) = sum(A_est*g>res.std*kLevel);
            end
        end
    end
end
...
pval_Ar = tcdf(-T, df2);
hval_Ar = pval_Ar < ip.Results.AlphaT;

% Assemble output struct from sliced arrays
pStruct = struct(...
    'x', x_out, 'y', y_out, 'A', A_out, 's', s_out, 'c', c_out, ...
    'x_pstd', x_pstd, 'y_pstd', y_pstd, 'A_pstd', A_pstd, 's_pstd', s_pstd, 'c_pstd', c_pstd, ...
    'x_init', reshape(xi, [1 np]), 'y_init', reshape(yi, [1 np]), ...
    'sigma_r', sigma_r, 'SE_sigma_r', SE_sigma_r, 'RSS', RSS, ...
    'pval_Ar', pval_Ar, 'mask_Ar', mask_Ar, 'hval_Ar', hval_Ar, 'hval_AD', hval_AD);