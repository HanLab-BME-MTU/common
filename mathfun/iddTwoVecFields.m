function S = iddTwoVecFields(Va,Vh,V,P)
...
%    cov(V1,Rh2) : The covariance between 'V1' and 'Rh2'.
%    cov(V2,Rh1) : The covariance between 'V2' and 'Rh1'.
%
% Data:
...
%    D9  : (= covVa1Vh1) The covariance between 'Va1' and 'Vh1'.
%    D10 : (= covVa2Vh2) The covariance between 'Va2' and 'Vh2'.
%    D11 : (= covVa1Va2) The covariance between 'Va1' and 'Va2'.
%    D12 : (= covVh1Vh2) The covariance between 'Vh1' and 'Vh2'.
%    D13 : (= covVa1Vh2) The covariance between 'Va1' and 'Vh2'.
%    D14 : (= covVa2Vh1) The covariance between 'Va2' and 'Vh1'.
...
x = x - x(1) + 1;
y = y - y(1) + 1;
img = ones(y(end),x(end));

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Divide the image area into blocks and assign a cluster value to each
% block. We then identify the block to which each vector belong.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
nGridY = length(y);
nGridX = length(x);
for jx = 1:nGridX-1
    for jy = 1:nGridY-1
        img(y(jy):y(jy+1),x(jx):x(jx+1)) = sub2ind([nGridY-1,nGridX-1],jy,jx);
    end
end

%Round P to integer pixel.
[imgH,imgW] = size(img);
subP = round(P);
iP = sub2ind(size(img),subP(:,2),subP(:,1));

aSpd = sqrt(sum(Va.^2,2));
anglVa = sign(Va(:,2)).*acos(Va(:,1)./aSpd);

nBlocks = (nGridX-1)*(nGridY-1);
b.pI    = cell(nBlocks,1);
b.angl  = NaN*ones(nBlocks,1);
b.CP    = NaN*ones(nBlocks,2);
b.V     = NaN*ones(nBlocks,2);
b.rotV  = NaN*ones(nBlocks,2);
for k = 1:nBlocks
    %Identify the center of the block.
    [yI xI]   = ind2sub([nGridY-1,nGridX-1],k);
    b.CP(k,:) = [(x(xI)+x(xI+1))/2 (y(yI)+y(yI+1))/2];
    
    b.pI{k} = find(img(iP) == k);
    
    if ~isempty(b.pI{k})
        b.angl(k) = sum(anglVa(b.pI{k},:))/length(b.pI{k});
        b.V(k,:)  = [cos(b.angl(k)) sin(b.angl(k))];
    
        %Assemble rotation matrix for this block.
        cosAngle = b.V(k,:)*[1;1]*sqrt(2)/2;
        sinAngle = b.V(k,:)*[-1;1]*sqrt(2)/2;
        rotM = [cosAngle sinAngle;-sinAngle cosAngle];

        Va(b.pI{k},:) = Va(b.pI{k},:)*rotM.';
        Vh(b.pI{k},:) = Vh(b.pI{k},:)*rotM.';
        V(b.pI{k},:)  = V(b.pI{k},:)*rotM.';
        
        %To check the correctness of the rotation, we also rotate b.V to
        % see if they all align with the vector [1 1].
...
%Plot grid lines
for k = 1:length(y)
    plot(x,y(k)*ones(size(x)),'w')
end
for k = 1:length(x)
    plot(x(k)*ones(size(y)),y,'w');
end

aSpd = sqrt(sum(V.^2,2));
meanSpd = mean(aSpd);
bvDispScale = meanSpd*dispScale*4/3;
quiver(P(:,1),P(:,2),Va(:,1)*dispScale,Va(:,2)*dispScale,0,'y');
quiver(P(:,1),P(:,2),Vh(:,1)*dispScale,Vh(:,2)*dispScale,0,'g');
quiver(b.CP(:,1),b.CP(:,2),b.V(:,1)*bvDispScale,b.V(:,2)*bvDispScale,0,'r');
quiver(b.CP(:,1),b.CP(:,2),b.rotV(:,1)*bvDispScale,b.rotV(:,2)*bvDispScale,0,'b');
...
% unitV(:,2) = V(:,2)./aSpd;
% 
% %Cosin of the rotation angle.
% cosAngle = unitV*[sqrt(2)/2;sqrt(2)/2];
% sinAngle = unitV*[-sqrt(2)/2;sqrt(2)/2];
% 
% rotVa = Va;
% rotVh = Vh;
% for k = 1:size(Va,1)
%    %Assemble rotation matrix
%    rotM = [cosAngle(k) sinAngle(k);-sinAngle(k) cosAngle(k)];
%    Va(k,:) = Va(k,:)*rotM.';
%    Vh(k,:) = Vh(k,:)*rotM.';
% end
...
D7  = var(Vh(:,1));
D8  = var(Vh(:,2));
D9  = mean((Va(:,1)-D1).*(Vh(:,1)-D5));
D10 = mean((Va(:,2)-D2).*(Vh(:,2)-D6));
D11 = mean((Va(:,1)-D1).*(Va(:,2)-D2));
D12 = mean((Vh(:,1)-D5).*(Vh(:,2)-D6));
D13 = mean((Va(:,1)-D1).*(Vh(:,2)-D6));
D14 = mean((Va(:,2)-D2).*(Vh(:,1)-D5));
