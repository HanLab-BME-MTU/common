function [C,T,res] = snakeBasedBezierFitUnderConstraint(data,n,beta,varargin)
...
res = sqrt(sum((B * C - data).^2,2));
  ...
  function [F, G, H] = fun(X)
    ...
    G1 = sum((C(2,:) - C(1,:)) .* (data(1,:) - C(1,:)), 2);
    G2 = sum((C(n-1,:) - C(n,:)) .* (data(m,:) - C(n,:)), 2);
    ...
    if nargout > 1
      ...
      % dL/dxk, k = 0
      G((0:(d-1)) * (n+1) + 1) = G((0:(d-1)) * (n+1) + 1) + (2 * C(1,:) - ...
        C(2,:) - data(1,:))';
      % dL/dxk, k = 1
      G((0:(d-1)) * (n+1) + 2) = G((0:(d-1)) * (n+1) + 2) + (data(1,:) - ...
        C(1,:))';
      % dL/dxk, k = n-1
      G((1:d) * (n+1) - 1) = G((1:d) * (n+1) - 1) + (data(m,:) - C(n+1,:))';
      % dL/dxk, k = n
      G((1:d) * (n+1)) = G((1:d) * (n+1)) + (2 * C(n+1,:) - C(n,:) - ...
        data(m,:))';
    ...
    if nargout > 2
      ...
      % Add the contraint's contributions.
      H(1,1) = H(1,1) + 2 * lambda1;
      H(2,1) = H(2,1) - lambda1;
      H(1,2) = H(2,1);
      H(end) = H(end) + 2 * lambda2;
      H(end-1,end) = H(end-1,end) - lambda2;
      ...
      for iDim = 1:d      
        ...
        H(end-m+1:end-2, offset:offset+n) = d2Fdtdx;
        H(offset:offset+n, end-m+1:end-2) = d2Fdtdx';
      end
      ...
      H((0:(d-1)) * (n+1) + 1, end-1) = 2 * C(1,:) - C(2,:) - data(1,:);
      H(end-1, (0:(d-1)) * (n+1) + 1) = H((0:(d-1)) * (n+1) + 1, end-1);
      
      H((0:(d-1)) * (n+1) + 2, end-1) = data(1,:) - C(1,:);
      H(end-1, (0:(d-1)) * (n+1) + 2) = H((0:(d-1)) * (n+1) + 2, end-1);
      
      % Compute d2Ldxdlambda2
      H((1:d) * (n+1) - 1, end) = data(m,:) - C(n+1,:);
      H(end, (1:d) * (n+1) - 1) = H((1:d) * (n+1) - 1, end);
      
      H((1:d) * (n+1), end) = 2 * C(n+1,:) - C(n,:) - data(m,:);
      H(end, (1:d) * (n+1)) = H((1:d) * (n+1), end);
    end
  end
end
