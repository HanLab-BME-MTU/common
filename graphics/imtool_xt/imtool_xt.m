function [hout,hImout] = imtool_xt(varargin)
%IMTOOL_XT Display image in the extended Image Tool.  
%
%   *NOTE:This is a modified version of matlabs imtool.m, which allows
%   plotting on top of the image and returns the handle to the image
%   object.*
%
%   IMTOOL_XT opens a new Image Tool in an empty state. Use the File menu options
%   "Open..." or "Import From Workspace..." to choose an image for display.
%
%   IMTOOL_XT_XT(I) displays the grayscale image I.
%
%   IMTOOL_XT_XT(I,[LOW HIGH]) displays the grayscale image I, specifying the display
...
%   [LOW HIGH], IMTOOL_XT_XT uses [min(I(:)) max(I(:))]; the minimum value in I
%   displays as black, and the maximum value displays as white.
%
%   IMTOOL_XT_XT(RGB) displays the truecolor image RGB.
%
%   IMTOOL_XT_XT(BW) displays the binary image BW. Values of 0 display as black, and
%   values of 1 display as white.
%
%   IMTOOL_XT_XT(X,MAP) displays the indexed image X with colormap MAP.
%
%   IMTOOL_XT_XT(FILENAME) displays the image contained in the graphics file FILENAME.
...
%
%   HFIGURE = IMTOOL_XT_XT(...) returns a handle HFIGURE to the figure
%   created by imtool
%   [HFIGURE,HIMAGE] = IMTOOL_XT_XT(...) returns a handle to the HFIGURE
%   and the image created by imtool
%
%   IMTOOL_XT_XT. CLOSE(HFIGURE) closes the Image Tool.
%
%   IMTOOL_XT_XT CLOSE ALL closes all instances of the Image Tool.
%
%   IMTOOL_XT_XT(...,PARAM1,VAL1,PARAM2,VAL2,...) displays the image, specifying
...
%                            The syntax IMTOOL_XT_XT(I,[LOW HIGH]) is equivalent to
%                            IMTOOL_XT_XT(I,'DisplayRange',[LOW HIGH]).
%                            The parameter name must be specified when 
%                            using IMTOOL_XT_XT with a filename, as in the syntax
%                            IMTOOL_XT_XT(FILENAME,'DisplayRange'[LOW HIGH]).
%
...
%                            large to display on the screen, IMTOOL_XT_XT displays
%                            the image at the largest magnification that fits 
%                            on the screen.
%
%                            When set to 'fit' IMTOOL_XT_XT scales the entire image
...
%   IMTOOL_XT_XT. R-Sets also improve performance of IMTOOL_XT_XT for large images.
%
%   Related Toolbox Preferences
%   ---------------------------
%   You can use the Image Processing Preferences dialog to set toolbox
%   preferences that modify the behavior of IMTOOL_XT_XT. To access the dialog,
...
%   IPTSETPREF function. IMTOOL_XT_XT preferences include:
%
...
%     'InitialMagnification' parameter when you call IMTOOL_XT_XT, as follows:
...
%   figure, axes, and image object property settings for image display. IMTOOL_XT_XT
...
%   tool. IMTOOL_XT_XT presents an integrated environment for displaying images and
...
%   $Revision: 1.1.10.42 $  $Date: 2012/01/16 16:49:25 $
 ...
 %suppressPlotTools(hFig);
  ...
  if (nargout > 0)
   ...
   function addImageToImtool(varargin)
     ...
     try
       ...
       % parse all arguments
       common_args = imageDisplayParsePVPairs(specificArgNames,varargin{:});
       ...
       % do not enforce xdata/ydata constrain for rsets
           ...
           if control_key_pressed
               switch (evt.Key)
                   case 'add'
                       ...
                       newMag = findZoomMag('in',currentMag);
                       sp_api.setMagnification(newMag)
                   case 'subtract'
                       currentMag = sp_api.getMagnification();
                       newMag = findZoomMag('out',currentMag);
                       sp_api.setMagnification(newMag)
               end
           end
   ...
   function createPanels
    ...
    bottomPanelHeight = 21; % initialize to reasonable height
     ...
     hBottomPanel = uipanel('Parent',hFig,...
                            ...
                            'BorderType','none',...
                            'Units','Pixels',...
                            'Position',[1 1 hFigPos(3) bottomPanelHeight]);
     ...
     % image or rset.
     im_width  = getImWidth(hIm);
     im_height = getImHeight(hIm);
     ...
     switch initial_mag
      ...
      case 'adaptive'
       ...
       if is_image_small
         ...
         hFigPos = getpixelposition(hFig); %refresh height after initSize
         ...
         % even if we decide to display at 100%
         onScreenImH = adaptiveMag * im_height;
   ...
   function toolbar = createMenusAndToolbar
     ...
     iconRoot = '/usr/local/matlab/toolbox/images/icons';
     iconRootMATLAB = '/usr/local/MATLAB/R2012b/toolbox/matlab/icons';     
  ...
  function resizeImtool(varargin)
    ...
    if (spH > 0) && (figPos(3) > 0)
        ...
        set(hScrollPanel,'Units','pixels','position', [1 spB figPos(3) spH]);
    end
