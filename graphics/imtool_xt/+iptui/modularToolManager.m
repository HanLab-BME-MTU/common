...
%   $Revision: 1.1.6.3 $ $Date: 2010/11/17 11:23:59 $
...
for i = 1:numel(tool_list)
    ...
    if ishghandle(this_tool)
        ...
        if isa(listeners,'event.listener')
            set(listeners,'Enabled',new_enabled_prop);
        elseif isa(listeners,'handle.listener')
            set(listeners,'Enabled',udd_prop);
        end
