...
ip.addParameter('Parent', gca, @ishandle);
ip.addParameter('Div', 1, @isscalar);
ip.addParameter('NormX', false, @islogical);
ip.addParameter('DisplayFunction', @(x) x, @(x) isa(x, 'function_handle'));
ip.parse(varargin{:});
xv = ip.Results.xv(:)';
yv = ip.Results.yv(:)';
