function makeuTrackPackage(outDir,uTrackDir,comDir)
...
%
%   uTrackDir - the directory you have checked out the biosensors project to.
% 
...
if nargin < 2 || isempty(uTrackDir)    
    uTrackDir = uigetdir(pwd,'Select uTrack dir:');
end
...
bioFuns = dir([uTrackDir filesep '*.m']);
...
bioExtras = dir([uTrackDir filesep '*.fig']);
bioExtras = {bioExtras(:).name}';
bioExtras = cellfun(@(x)([uTrackDir filesep x]),bioExtras,'UniformOutput',false);
